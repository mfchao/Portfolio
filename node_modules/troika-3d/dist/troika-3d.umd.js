(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.troika_3d = {}, global.THREE, global.troika_core, global.troika_three_utils, global.React, global.PropTypes));
  ...
  const {assign, forOwn} = troikaCore.utils;
  const singletonVec3 = new three.Vector3();
  const singletonVec3b = new three.Vector3();
  ...
  class Object3DFacade extends troikaCore.PointerEventTarget {
    ...
    getWorldPosition(vec3 ) {
      this.updateMatrices();
      return (vec3 || new three.Vector3()).setFromMatrixPosition(this.threeObject.matrixWorld)
    }
  ...
  const lookAtUp = new three.Vector3(0, 1, 0);
    ...
    class Camera3DFacade extends Object3DFacade {
      ...
      updateMatrices() {
        ...
        if (matrixVersionBeforeUpdate !== this._worldMatrixVersion) {
          troikaThreeUtils.invertMatrix4(camObj.matrixWorld, camObj.matrixWorldInverse);
        }
      }
      ...
      getFrustum() {
        ...
        if (frustum._lastWorldMatrixVersion !== _worldMatrixVersion || frustum._lastProjMatrixVersion !== _projectionMatrixVersion) {
          let camObj = this.threeObject;
          let matrix = new three.Matrix4().multiplyMatrices(camObj.projectionMatrix, camObj.matrixWorldInverse);
      ...
      getRayAtProjectedCoords(u, v) {
        ...
        const ray = tempRaycaster.ray = new three.Ray();
  ...
  const PerspectiveCamera3DFacade = createCameraFacade(three.PerspectiveCamera, ['fov', 'aspect', 'near', 'far'], ['focus', 'filmGauge', 'filmOffset']);
  const OrthographicCamera3DFacade = createCameraFacade(three.OrthographicCamera, ['left', 'right', 'top', 'bottom', 'near', 'far']);
  ...
  const AmbientLight3DFacade = createLightFacade(three.AmbientLight);
  const DirectionalLight3DFacade = createLightFacade(three.DirectionalLight, three.DirectionalLightHelper);
  const SpotLight3DFacade = createLightFacade(three.SpotLight, three.SpotLightHelper);
  const PointLight3DFacade = createLightFacade(three.PointLight, three.PointLightHelper);
  const HemisphereLight3DFacade = createLightFacade(three.HemisphereLight, three.HemisphereLightHelper, {
    groundColor: {
      set(c) {
  ...
  const RectAreaLight3DFacade = createLightFacade(three.RectAreaLight);
...
`;
  ...
  function getInstancingDerivedMaterial(baseMaterial, instanceUniforms) {
    ...
    if (!derived || derived._instanceUniformsKey !== instanceUniformsKey) {
      derived = troikaThreeUtils.createDerivedMaterial(baseMaterial, {
        defines: {
...
${fragmentShader}`;
  ...
  const { assign: assign$1 } = troikaCore.utils;
        ...
        modelViewMatrix: {
          value: function() {
            ...
            modelViewMatrix.multiplyMatrices = function(viewMatrix, matrixWorld) {
              ...
              return three.Matrix4.prototype.multiplyMatrices.call(this, viewMatrix, matrixWorld)
            };
  ...
  class BatchGeometryPool {
    ...
    borrow(key, baseGeometry, instanceUniformSizes) {
      ...
      if (!batchGeometry) {
        ...
        for (let row = 0; row < 3; row++) {
          let attr = new three.InstancedBufferAttribute(new Float32Array(INSTANCE_BATCH_SIZE * 4), 4);
          if (attr.setUsage) {
        ...
        for (let name in instanceUniformSizes) {
          ...
          let attr = new three.InstancedBufferAttribute(new Float32Array(INSTANCE_BATCH_SIZE * itemSize), itemSize);
          if (attr.setUsage) {
            attr.setUsage(DYNAMIC_DRAW);
          } else {
            attr.dynamic = true;
  ...
  const INFINITE_SPHERE = new three.Sphere(undefined, Infinity);
  const tempArr = [null];
  ...
  class Scene3DFacade extends Object3DFacade {
      ...
      if (lights) {
        ...
        lights.forEach((def, i) => {
          ...
          if (typeof facade === 'function') {
            let realDef = troikaCore.utils.assign({}, def);
            delete realDef.type;
    ...
    set fog(def) {
      ...
      if (def) {
        ...
        let fogClass = isExp2 ? three.FogExp2 : three.Fog;
  ...
  const { assign: assign$2, forOwn: forOwn$1 } = troikaCore.utils;
  ...
  const { assign: assign$3 } = troikaCore.utils;
  const tmpVec2 = new three.Vector2();
  const tmpVec3 = new three.Vector3();
  const raycaster = new three.Raycaster();


  class World3DFacade extends troikaCore.WorldBaseFacade {
    constructor(canvas) {
      super(canvas);
    ...
    afterUpdate() {
      ...
      const RendererClass = this.rendererClass || three.WebGLRenderer;
      ...
      if (backgroundColor !== this._bgColor) {
        renderer.setClearColor(new three.Color(backgroundColor || 0), backgroundColor != null ? 1 : 0);
        this._bgColor = backgroundColor;
      }

      renderer.outputEncoding = this.outputEncoding || three.LinearEncoding;
      renderer.toneMapping = this.toneMapping || three.NoToneMapping;
  ...
  function makeWorldTextureProvider(WrappedFacadeClass) {
    ...
    return class WorldTextureProvider extends WrappedFacadeClass {
      constructor(parent) {
        super(parent);
        this.worldTexture = new three.CanvasTexture(); //no canvas yet, will be added in first afterUpdate()
      ...
      afterUpdate() {
        ...
        if (!innerWorld || !newWorldConfig || !(innerWorld instanceof newWorldConfig.facade)) {
          ...
          if (newWorldConfig) {
            ...
            this.worldTexture = new three.CanvasTexture(canvas);
        ...
        if (innerWorld) {
          ...
          troikaCore.utils.assign(innerWorld, newWorldConfig, {pixelRatio: 1});
    ...
    afterUpdate() {
      ...
      if (material !== this._lastMtl) {
        ...
        if (typeof material === 'string') {
          material = new (MESH_MATERIALS[material] || three.MeshStandardMaterial)();
        }
   ...
   */
  const getBoxGeometry = troikaCore.utils.memoize(() => {
    return new three.BoxGeometry(1, 1, 1, 1, 1)
  });
  ...
  const geometries = Object.create(null, [
    ...
    descr[name] = {
      get: troikaCore.utils.memoize(() =>
        new three.CircleGeometry(1, segments).rotateX(-Math.PI / 2)
      )
    };
    return descr
  }, {}));
  ...
  class CircleFacade extends MeshFacade {
    constructor (parent) {
      super(parent);
      this['material.side'] = this['material.shadowSide'] = three.DoubleSide;
    }
  ...
  const getGeometry = troikaCore.utils.memoize(() => {
    return new three.PlaneGeometry(1, 1, 1, 1).rotateX(-Math.PI / 2)
  });
  ...
  const geometries$1 = Object.create(null, [
    ...
    descr[name] = {
      get: troikaCore.utils.memoize(() => new three.SphereGeometry(1, wSegs, hSegs))
    };
    return descr
  }, {}));
  ...
  class Canvas3D extends troikaCore.ReactCanvasBase {
    constructor(props) {
      super(props);
  ...
  Canvas3D.propTypes = troikaCore.utils.assignIf(
    {
      ...
      background: T__default['default'].any,
      environment: T__default['default'].any,
      outputEncoding: T__default['default'].number,
      toneMapping: T__default['default'].number,
      lights: T__default['default'].array,
      camera: T__default['default'].object,
      fog: T__default['default'].object,
      objects: T__default['default'].oneOfType([T__default['default'].array, T__default['default'].object]).isRequired,
      antialias: T__default['default'].bool,
      onBackgroundClick: T__default['default'].func,
      rendererClass: T__default['default'].func
    },
   ...
   */
  Canvas3D.contextType = React__default['default'].createContext({
  ...
  exports.BoxFacade = BoxFacade;
  exports.Canvas3D = Canvas3D;
  exports.CircleFacade = CircleFacade;
  exports.CubeFacade = CubeFacade;
  exports.DirectionalLight3DFacade = DirectionalLight3DFacade;
  exports.Group3DFacade = Group3DFacade;
  exports.HemisphereLight3DFacade = HemisphereLight3DFacade;
  ...
  exports.RectAreaLight3DFacade = RectAreaLight3DFacade;
  exports.Scene3DFacade = Scene3DFacade;
  exports.SphereFacade = SphereFacade;
